...
const Filtro = ({ filtroPorPizza, filtroPorHamburgueres, milkShake, hotDogs }) => {
  return (
    <div className="max-w-3xl mx-auto px-1 bg-white rounded-lg shadow-md">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
        <button onClick={filtroPorPizza} className="flex items-center  p-4 rounded-lg transition duration-200">
          <IoPizzaOutline className="text-4xl text-red-600 " />
          <span className="text-gray-700">Pizzas</span>
        </button>

        <button onClick={filtroPorHamburgueres} className="flex  items-center  p-4 rounded-lg transition duration-200">
          <PiHamburgerBold className="text-4xl text-yellow-600" />
          <span className="text-gray-700">Hambúrgueres</span>
        </button>

        <button onClick={hotDogs} className="flex  items-center  p-4 rounded-lg transition duration-200">
          <LiaHotdogSolid className="text-4xl text-orange-600" />
          <span className="text-gray-700">Hot Dogs</span>
        </button>

        <button onClick={milkShake} className="flex  items-center  p-4 rounded-lg transition duration-200">
